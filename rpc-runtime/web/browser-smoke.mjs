...
try {
  for (const [label, viewport] of [["desktop", { width: 1440, height: 1000 }], ["mobile", { width: 390, height: 844 }]]) {
    ...
    try {
      ...
      await page.locator("#expand-prompt").click();
      assert.equal(await page.locator("#expand-prompt").getAttribute("aria-expanded"), "true");
      assert.ok(await promptHeight() > 100);
      await page.evaluate(() => {
        const spacer = document.createElement("div");
        spacer.id = "composer-scroll-fixture"; spacer.style.height = "3000px";
        document.querySelector("#messages").append(spacer);
        const conversation = document.querySelector("#conversation");
        conversation.scrollTop = conversation.scrollHeight;
      });
      await page.waitForTimeout(100);
      await page.locator("#conversation").evaluate(node => { node.scrollTop -= 700; });
      await page.waitForFunction(() => document.querySelector(".compose-area").classList.contains("compact"));
      assert.equal(await promptHeight(), 40);
      assert.equal(await page.locator("#send").isVisible(), true);
      assert.equal(await page.locator("#model").isVisible(), false);
      assert.equal(await page.locator("#prompt").inputValue(), "Draft while syncing");
      await page.waitForTimeout(100);
      assert.equal(await promptHeight(), 40, "resizing must not toggle the composer back open");
      await noOverflow(page);
      const jump = await page.locator("#jump-latest").boundingBox();
      const compose = await page.locator(".compose-area").boundingBox();
      assert.ok(jump && jump.y + jump.height <= compose.y, "jump button stays above composer");
      await page.locator("#prompt").click();
      assert.ok(await promptHeight() > 100, "editing restores the expanded size");
      await page.locator("#expand-prompt").click();
      assert.equal(await promptHeight(), 44);
      await page.locator("#conversation").evaluate(node => { node.scrollTop -= 100; });
      await page.waitForFunction(() => document.querySelector(".compose-area").classList.contains("compact"));
      await page.locator("#jump-latest").click();
      assert.equal(await promptHeight(), 44);
      await page.evaluate(() => document.querySelector("#composer-scroll-fixture").remove());
      assert.equal(await page.locator("#prompt").inputValue(), "Draft while syncing");
      ...
      await rows.filter({ hasText: "Pi Fast" }).locator("input").uncheck();
      assert.equal(await page.locator("#model option").filter({ hasText: "Pi Fast" }).count(), 1, "current model remains visible");
      await page.locator("#model-visibility-search").fill("reasoning");
      assert.equal(await rows.count(), 1, "model visibility is searchable");
      await page.locator("#hide-profile-section").check();
      assert.equal(await page.locator("#profile-section").isVisible(), false);
      assert.equal(await page.locator("#profile-filter").inputValue(), "");
      const preferences = await page.evaluate(() => ({ models: JSON.parse(localStorage.getItem("agentbox.pi.hidden-models.v1")), profile: localStorage.getItem("agentbox.pi.hide-profile.v1") }));
      assert.equal(preferences.models.length, 2); assert.equal(preferences.profile, "true");
      await page.locator("#close-settings").click(); await openSettings();
      assert.equal(await rows.filter({ hasText: "Pi Reasoning" }).locator("input").isChecked(), false);
      await page.locator("#show-all-models").click();
      assert.equal(await page.locator("#model option").count(), 2);
      await page.locator("#hide-profile-section").uncheck();
      await page.locator("#close-settings").click();
      assert.equal(await page.locator(".brand-mark").evaluate((node) => node.getBoundingClientRect().width), 28);
